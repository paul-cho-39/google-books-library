...
const DeleteButton = ({ userId, book }: ButtonProps) => {
  ...
  const dataBooks = queryClient.getQueryData<QueryData>(
      ...
      onSuccess: () => {
        ...
        queryClient.setQueryData(queryKeys.userLibrary(userId), {
          ...
          library: {
            ...
            currentlyReading: filterId(
              dataBooks?.library?.currentlyReading as string[],
              id
            ),
            wantToRead: filterId(
              dataBooks?.library?.wantToRead as string[],
              id
            ),
          },
        });
